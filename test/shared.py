...
def run(*args, **kwargs):  # eval_str, msg, slow, globals_, f= None, fast= False, stop= False, keep= False):
    return _run("{t}()", *args, **kwargs)


def run_keep(*args, **kwargs):  # eval_str, msg, slow, globals_, f= None, fast= False, stop= False, keep= False):
    return _run("{t}({keep})", *args, **kwargs)


def _run(eval_str, msg, slow, globals_, f=None, fast=False, stop=False, keep=False):
    ...
    if f is None:
        ...
        for t in tests:
            ...
            result = eval(eval_str.format(t=t, keep=keep), globals_)
            test_ct += 1
            if result:
                ok_ct += 1
            else:
